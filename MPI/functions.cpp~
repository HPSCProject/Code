...
void init_gaussian(qr_type fIn[NX*NY*Q], qr_type fOut[NX*NY*Q], qr_type rho[NX*NY], qr_type ux[NX*NY], qr_type uy[NX*NY], const double wi[Q],int l_start_idx,int l_end_idx)
{
  qr_type fEq;

  for (size_t i = l_start_idx; i < l_end_idx; ++i)
  {
...
void write_gaussian(qr_type rho[NX*NY], qr_type ux[NX*NY], qr_type uy[NX*NY], const size_t& ts)
{
  ...
  printf(fname, "data/Xrho_t%i.dat", ts);
  ...
  printf(fname, "data/Yrho_t%i.dat", ts);
  out.open(fname, ios::out);
  for( j = 0; j < NY; ++j)
  {
  ...
  printf(fname, "data/Xux_t%i.dat", ts);
  out.open(fname, ios::out);
  for( i = 0; i < NX; ++i)
  {
    out << (i - MIDDLE_X) * SIN_V << "\t";
  ...
  printf(fname, "data/Yuy_t%i.dat", ts);
  out.open(fname, ios::out);
  for( j = 0; j < NY; ++j)
  {
    out << (j - MIDDLE_Y) * SIN_V << "\t";
